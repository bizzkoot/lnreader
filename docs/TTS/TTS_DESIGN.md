...
| **Paused Scroll Behavior**     | Modal  | Ask / Auto-change / Never            | Action when user scrolls up *while paused* (Manual Mode Prompt).                                                                 |
| **Playback Scroll Behavior**   | Modal  | Continue / Pause                     | Action when user scrolls *during* playback.                                                                                      |
...
4.  **Synchronization**: `addToBatch` is used to append to the Native queue without stopping playback. 
    -   *Note*: This mitigates the risk of `onQueueEmpty` firing prematurely if the refill is fast enough.
    -   *Race Condition*: If the queue runs dry *before* the refill completes (e.g., short paragraphs), `onQueueEmpty` will fire. Logic exists to ignore this if a refill is `inProgress`.
...
-   **Manual Scroll**:
    -   If user scrolls *during* playback:
...
3.  **Next Chapter (Screen Off)**:
    ...
    -   **Direct Extraction**: RN extracts paragraphs from the *new* HTML (passed via props) using `extractParagraphs` (RegEx/Soup).
