...
function simulateWakeResume(paragraphs, startIndex) {
  console.log(
    ...
    }] Screen wake detected at paragraph ${startIndex}`,
  );
  ...
  console.log(
    `  Session incremented to ${state.ttsSessionRef.current}, queue cleared`,
  );
  ...
  console.log(
    `  New queue: startIndex=${state.ttsQueueRef.current.startIndex}, ` +
...
function simulateOnSpeechDone() {
  ...
  if (
    !state.ttsQueueRef.current ||
    state.currentParagraphIndexRef.current < 0
  ) {
  ...
  if (currentIdx < queueStart) {
    console.log(
      `  onSpeechDone: currentIdx ${currentIdx} < queueStart ${queueStart}, deferring`,
    );
  ...
  if (currentIdx >= queueEnd) {
    console.log(
      `  onSpeechDone: currentIdx ${currentIdx} >= queueEnd ${queueEnd}, need new queue`,
    );
  ...
  if (nextIndex <= state.currentParagraphIndexRef.current) {
    console.log(
      `  onSpeechDone: WARNING - Index not advancing! next=${nextIndex}`,
    );
  }

  state.currentParagraphIndexRef.current = nextIndex;
  console.log(
    `  onSpeechDone: Advanced to ${nextIndex} (queue: ${queueStart}-${
      queueEnd - 1
    })`,
  );
...
function simulateStaleTtsQueue(startIndex, paragraphs) {
  ...
  const timeSinceWakeResume =
    Date.now() - state.wakeResumeGracePeriodRef.current;

  // BUG FIX: Wake resume grace period
  if (timeSinceWakeResume < 500 && state.wakeResumeGracePeriodRef.current > 0) {
    console.log(
      `  REJECTED: Wake grace period (${timeSinceWakeResume}ms < 500ms)`,
    );
  ...
  if (currentIdx >= 0 && startIndex < currentIdx) {
    console.log(
      `  REJECTED: Stale queue (starts at ${startIndex}, currently at ${currentIdx})`,
    );
  ...
  if (currentIdx >= 0 && startIndex > currentIdx + 1) {
    console.log(
      `  WARNING: Queue gap detected (starts at ${startIndex}, currently at ${currentIdx})`,
    );
  }
...
function runTests() {
  ...
  if (blockedResult.blocked && blockedResult.reason === 'wake_transition') {
    console.log(
      '✓ PASS: onSpeechDone correctly blocked during wake transition',
    );
    testsPassed++;
  } else {
