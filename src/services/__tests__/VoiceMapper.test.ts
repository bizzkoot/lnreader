...
describe('VoiceMapper', () => {
    it('should resolve by native ID (Android)', () => {
        ...
        expect(mapping?.displayName).toContain('ENGLISH (US) Female voice 2 (US) HIGH');
    });

    it('should resolve by full name (Microsoft)', () => {
        const mapping = getVoiceMapping('Microsoft Jenny Online (Natural) - English (United States)');
        expect(mapping).toBeDefined();
    ...
    it('should resolve by alternative ID', () => {
        ...
        const mapping = getVoiceMapping('Microsoft Emma Online (Natural) - English (United States)');
        expect(mapping).toBeDefined();
